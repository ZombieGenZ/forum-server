  ...
  checkSchema(
    {
      email_or_phone_or_username: {
        ...
        custom: {
          options: async (value, { req }) => {
            ...
            if (!result) {
              throw new Error(
                MESSAGE.USER_MESSAGE.EMAIL_OR_PHONE_OR_USERNAME_OR_PASSWORD_IS_NOT_VALID
              )
            }
      ...
      password: {
        notEmpty: {
          errorMessage:
            MESSAGE.USER_MESSAGE.PASSWORD_IS_REQUIRED
        },
        trim: true,
        isString: {
          errorMessage:
            MESSAGE.USER_MESSAGE.PASSWORD_MUST_BE_A_STRING
        }
      }
    },
    ['body']
